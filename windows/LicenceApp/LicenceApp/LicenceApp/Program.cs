        ...
        try
        {
             ...
             if (getLicence != null)
            {
                if (rsaEncryption.DecryptLicence(getLicence, privateKey, deviceInfo.getInfoBytes()))
                {
                    ...
                    File.WriteAllBytes(FileManager.timePublicKeyPath, ProtectedData.Protect(pubKey, null, DataProtectionScope.CurrentUser));
                    File.WriteAllBytes(FileManager.timePrivateKeyPath, ProtectedData.Protect(privKey, null, DataProtectionScope.CurrentUser));
                    File.WriteAllBytes(FileManager.initialTimePath, ProtectedData.Protect(encData, null, DataProtectionScope.CurrentUser));
                    File.WriteAllBytes(FileManager.setUpTime, ProtectedData.Protect(encData, null, DataProtectionScope.CurrentUser));
                    File.WriteAllBytes(FileManager.timeDeltaPath, ProtectedData.Protect(encTimeDelta, null, DataProtectionScope.CurrentUser));
