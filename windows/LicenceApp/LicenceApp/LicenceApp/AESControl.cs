    ...
    internal class AESControl
    {
        ...
        public AESControl(byte[] deviceInfo,string state)
        {
            ...
            if (isFilesCorrupted())
            {
                ...
                this.encryptedTextFromFile = ProtectedData.Unprotect(File.ReadAllBytes(FileManager.AESTextPath), null, DataProtectionScope.CurrentUser);
        ...
        public async Task<bool> OnlineAESCheck()
        {
            ...
            if (state == "1")
            {//web kontrol etmiş
                byte[] encAES= ProtectedData.Unprotect(File.ReadAllBytes(FileManager.AESTextPath), null, DataProtectionScope.CurrentUser);
            ...
            {//offline kontrol olmuş
                keyIv = ProtectedData.Unprotect(File.ReadAllBytes(FileManager.AESKeyIVPath), null, DataProtectionScope.CurrentUser);
        ...
        public void WriteKeyAndIvToFile(byte[] key, byte[] iv)
        {
            ...
            File.WriteAllBytes(FileManager.AESKeyIVPath, ProtectedData.Protect(combined, null, DataProtectionScope.CurrentUser));
        }
        private static (byte[] key, byte[] iv) ReadKeyAndIvFromFile()
        {
            byte[] combined = ProtectedData.Unprotect(File.ReadAllBytes(FileManager.AESKeyIVPath), null, DataProtectionScope.CurrentUser);
