...
public class RSAEncryption
{
        ...
        using (var rsa = new RSACryptoServiceProvider(1024))
        {
            try
            {
                if (File.Exists(FileManager.primaryPublicKeyPath))
                {
                    ...
                    File.WriteAllBytes(FileManager.privateKey, ProtectedData.Protect(privateKey, null, DataProtectionScope.CurrentUser));
                    if (onlineStatus)
                    {
    ...
    public bool DecryptLicence(byte[] encryptedText, byte[] privateKey, byte[] deviceInfo)
    {
        using (var rsa = new RSACryptoServiceProvider(1024))
        {
            ...
            if (aes.Control(deviceInfo, separatedKey, separatedIV, licenceData))
            {
                ...
                File.WriteAllBytes(FileManager.AESTextPath, ProtectedData.Protect(licenceData, null, DataProtectionScope.CurrentUser));
                return true;
            }
            else
            {
    ...
    {
        byte[] privateKey = ProtectedData.Unprotect(File.ReadAllBytes(FileManager.privateKey), null, DataProtectionScope.CurrentUser);
        using (var rsa = new RSACryptoServiceProvider(1024))
        {
            rsa.ImportCspBlob(privateKey);
